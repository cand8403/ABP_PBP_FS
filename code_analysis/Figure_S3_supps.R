...
load(paste0("code_analysis/analysis_output/surfaces_list_M5_imputed_",year.plot,"data.rdata"))
load(paste0("code_analysis/analysis_output/surfaces_list_M60_imputed_",year.plot,"data.rdata"))
load(paste0("code_analysis/analysis_output/surfaces_list_F5_imputed_",year.plot,"data.rdata"))
load(paste0("code_analysis/analysis_output/surfaces_list_F60_imputed_",year.plot,"data.rdata"))
...
save(predictions_list,file=paste0("code_analysis/analysis_output/Surface_2_predictions_list_imputed_",plotyear,"data.rdata"))
...
for(k in 1:3){
  ...
  predictions.females$e0<-as.numeric(predictions.females$e0)
  
  predictions.males$e0<-as.numeric(predictions.males$e0)
  ...
  e0_list_M[[k]]<-ggplot(predictions.males, aes(x=x, y=y)) +
    geom_raster(aes(fill=as.numeric(e0)), show.legend=T, interpolate=F, na.rm=T) +
    ...
    theme(plot.title=element_text(size=25,margin=unit(c(1,1,1,1),"cm"),hjust=0.5,face="bold"),title=element_text(size=25,margin=unit(c(1,1,1,1),"cm")),legend.key.size=unit(2,"cm"),legend.text = element_text(size=25),axis.title.y = element_text(margin=unit(c(1,1,1,1),"cm"))
    )	+
    ylim(yaxis[[k]])	+
    xlim(xaxis[[k]]) 
  ...
  e0_list_F[[k]]<-ggplot(predictions.females, aes(x=x, y=y)) +
    ...
    labs(x = labels[1], y = labels[2], title=paste0(labels[3], " = ", round(quantile(dataset.plot[, XYZ_list[[k]][3]])[3]))) +
    theme(axis.text=element_text(size=25), axis.title=element_text(size=25)) +
    theme(plot.title=element_text(size=25,margin=unit(c(0.5,0.5,0.5,0.5),"cm"),hjust=0.5,face="bold"),title=element_text(size=25),legend.key.size=unit(2,"cm"),legend.text = element_text(size=25),axis.title.y = element_text(margin=unit(c(1,1,1,1),"cm")))	+
    ylim(yaxis[[k]])	+
    xlim(xaxis[[k]])
}	


save(e0_list_M, file=paste0("code_analysis/analysis_output/Surface_2_list_male_imputed_",year.plot,"data.rdata"))
save(e0_list_F, file=paste0("code_analysis/analysis_output/Surface_2_list_females_imputed_",year.plot,"data.rdata"))

library(cowplot)
title <- ggdraw() +
  draw_label("Life Expectancy at Birth (Imputed Dataset)",
...
CairoPDF(paste0("code_analysis/analysis_output/Figures/Surface_2_",plotyear,"data_imputed.pdf"), height=15, width=23)
